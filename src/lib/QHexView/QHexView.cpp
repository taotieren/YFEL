...
QHexView::QHexView(QWidget *parent)
  ...
  // Set up the palette for better dark/light theme support
...
void QHexView::updatePositions() {
  ...
  int serviceSymbolsWidth =
      ADR_LENGTH * m_charWidth + GAP_ADR_HEX + GAP_HEX_ASCII;

  m_bytesPerLine = (width() - serviceSymbolsWidth) / (4 * m_charWidth) -
                   1; // 4 symbols per byte
...
void QHexView::paintEvent(QPaintEvent *event) {
  ...
  // Improved theme detection and color scheme
  bool isDarkTheme = palette().color(QPalette::Base).lightness() < 128;
  
  // Define colors for both themes
  QColor backgroundColor, textColor, addressAreaColor, separatorColor, selectionColor, cursorColor;
  
  if (isDarkTheme) {
    // Dark theme colors
    backgroundColor = QColor(45, 45, 45);        // #2D2D2D
    textColor = Qt::white;
    addressAreaColor = QColor(61, 61, 61);       // #3D3D3D
    separatorColor = QColor(85, 85, 85);         // #555555
    selectionColor = QColor(0, 120, 215);        // #0078D7
    cursorColor = Qt::white;
  } else {
    // Light theme colors
    backgroundColor = Qt::white;
    textColor = Qt::black;
    addressAreaColor = QColor(212, 212, 212);    // #D4D4D4
    separatorColor = Qt::gray;
    selectionColor = QColor(109, 158, 255);      // #6D9EFF
    cursorColor = Qt::black;
  }

  painter.fillRect(event->rect(), backgroundColor);

  painter.fillRect(QRect(m_posAddr, event->rect().top(),
                         m_posHex - GAP_ADR_HEX + 2, height()),
                   addressAreaColor);
  ...
  for (int lineIdx = firstLineIdx, yPos = yPosStart; lineIdx < lastLineIdx;
    ...
    QString address =
        QString("%1").arg(lineIdx * m_bytesPerLine, 10, 16, QChar('0'));
    ...
    for (int xPosAscii = m_posAscii, i = 0;
      ...
      // Draw ASCII with same background handling as hex values
      std::size_t pos = (lineIdx * m_bytesPerLine + i) * 2;
      if (pos >= m_selectBegin && pos < m_selectEnd) {
        painter.setBackground(selected);
        painter.setBackgroundMode(Qt::OpaqueMode);
  ...
  if (hasFocus()) {
    ...
    painter.fillRect(cursorX, cursorY, 2, m_charHeight, cursorColor);
  }
}
...
QByteArray QHexView::getData() { return m_pdata->getData(0, m_pdata->size()); }
