...
void ChipOP::chip_reset_chip() {
  ...
  } catch (const usb_bulk_recv_error
               &error) { // Catch usb_bulk_recv_error and handle as success
    qDebug() << "Reset Done";
  } catch (const usb_bulk_send_error
               &error) { // Catch usb_bulk_send_error and handle as success
    qDebug() << "Reset Done";
  }
}
...
chip_t ChipOP::get_current_chip() { return current_chip->get_chip_info(); }

bool ChipOP::check_chip() {
  // Enable long connection of FEL
  fel_->fel_open_connection();
  for (const auto &item : std::as_const(chip_db)) {
    qDebug() << "Checking chip 0x"
             << QString::number(item->get_chip_info().chip_id, 16);
...
void ChipOP::chip_exec(uint32_t addr) { fel_->fel_exec(addr); }
  ...
  try {
    ...
    if (spinand.get_spi_nand_size() == 0) {
      ...
      return spinand.get_spi_nand_name() + " " +
             QString::number(spinand.get_spi_nand_size() / 1024 / 1024) +
             "MB 0x" + QString::number(spinand.get_spi_nand_size(), 16);
    }
...
void ChipOP::chip_erase_spi_nand(uint32_t addr, uint32_t len) {
  ...
  try {
    ...
    if ((len + addr) > spinand.get_spi_nand_size()) {
      // If len + addr > spi nand size, disconnect signal, emit release ui and
      // throw exception
      disconnect(&spinand, &spi_nand::release_ui, this,
                 &ChipOP::chip_release_ui);
      fel_->fel_close_connection();
      emit release_ui();
...
QByteArray ChipOP::chip_read_spi_nand(uint64_t addr, uint64_t len) {
  ...
  return QByteArray::fromRawData(reinterpret_cast<char *>(buf),
                                 static_cast<int64_t>(len));
}

void ChipOP::chip_write_spi_nand(const uint64_t addr, const QByteArray &buf,
                                 const uint64_t len) {
  ...
  try {
    ...
    if (spi_nor.get_spi_nor_size() < 1) {
      ...
      if (spi_nor.get_spi_nor_name() == "SFDP") {
        return "JEDEC SFDP Compatible SPI NOR " +
               QString::number(spi_nor.get_spi_nor_size() / 1024 / 1024) +
